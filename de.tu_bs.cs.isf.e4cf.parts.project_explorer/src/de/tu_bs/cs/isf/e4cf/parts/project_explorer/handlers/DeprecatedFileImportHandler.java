...
import de.tu_bs.cs.isf.e4cf.parts.project_explorer.wizards.DeprecatedFileImportWizard;
 ...
 */
public class DeprecatedFileImportHandler {
	...
	public void execute(RCPImageService imageService, RCPDialogService dialogService, RCPSelectionService selectionService,
		...
		try {
			...
			DeprecatedFileImportWizard wizard = buildFileImportWizard(shell, imageService);
	...
	private DeprecatedFileImportWizard buildFileImportWizard(Shell shell, RCPImageService imageService) {
		ImageDescriptor image = imageService.getImageDescriptor(null, FILE_ICON_PATH);
		DeprecatedFileImportWizard wizard = new DeprecatedFileImportWizard(shell, image);
		return wizard;
	}
